    ...
    if data_path and os.path.exists(data_path):
        with open(data_path, 'r') as f:
            if data_path.endswith('.json'):
                data = json.load(f)
                return [item.get('query', '') for item in data if 'query' in item]
            else:
...
def evaluate_model(model_name, model_path, state, attack_queries_list, results_dir):
    print(f"\n==============================================")
    print(f"Evaluating Model: {model_name} | State: {state}")
    print(f"==============================================")
    ...
    perturbation_types = attack_queries_list[0].keys()
    results = []

    for i, _ in enumerate(attack_queries_list):
        results.append({
            "original_index": i,
            "perturbations": {}
        })
    ...
    for p_type in perturbation_types:
        ...
        prompts = [q_dict[p_type] for q_dict in attack_queries_list]
    ...
    output_filename = os.path.join(results_dir, f"{model_name}_{state}_results.json")
...
def main():
    ...
    clean_queries = load_queries(args.data_file)
    ...
    for query in tqdm(clean_queries):
        perms = {
            ...
            "indirect": atk_indirect.generate_attack(query),
            "gradient_suffix": atk_gradient.generate_attack(query)
        }
        attack_queries_list.append(perms)

    print("Permutations generated. Total queries prepared:", len(attack_queries_list))
    ...
    evaluate_model(
        ...
        state=args.state,
        attack_queries_list=attack_queries_list,
