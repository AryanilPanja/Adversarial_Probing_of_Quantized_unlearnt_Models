...
# Convert to absolute paths if they are provided so cd doesn't break them
if [[ "$DATA_FILE" != /* ]]; then
...
declare -a ta_models=(
    ...
    "ta_int4|../models/task_arithmetic/int4_unlearned_model|auto"
    "ta_int8|../models/task_arithmetic/int8_unlearned_model|auto"
)
...
for model_info in "${all_models[@]}"; do
    ...
    # Check if directory exists and has config.json (a real model) AND has actual weight files
    if [ -d "$model_path" ] && [ -f "$model_path/config.json" ]; then
        if ls "$model_path"/*.safetensors >/dev/null 2>&1 || ls "$model_path"/*.bin >/dev/null 2>&1; then
            echo ""
            echo "---------------------------------------------------------"
            ...
            echo ""
            echo "Skipping $model_name - Has config.json but missing weight files (*.safetensors or *.bin) in: $model_path"
        fi
    else
        echo ""
